...
from corpscanada.corpscanada_pipelines import corpscanada_root_pipeline
...
try:
    ...
    (post_credential_pipeline, success) = data_integration.pipelines.find_node(['initialization_and_load_tasks','corpscanada_credential_poster'])
    if success:
        creds_ct = 0
        ...
        while 0 < creds_ct:
            ...
            with EventProcessor() as eventprocessor:
                creds_ct = eventprocessor.get_outstanding_creds_record_count()
            log_info("Ran corpscanada_credential_poster for " + str(creds_ct) + " corps")
    else:
        print("Pipeline not found")
        log_error("Pipeline not found for:" + "corpscanada_credential_poster")
except Exception as e:
    print("Exception", e)
    log_error("corpscanada_credential_poster processing exception: " + str(e))
    raise
