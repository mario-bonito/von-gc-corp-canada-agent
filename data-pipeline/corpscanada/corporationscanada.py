...
system_type = 'CORPS_CAN'

CORPORATIONS_CANADA_TABLE_PREFIX = 'corporations_canada.'
...
class CorporationsCanada:
    ...
    def __init__(self, cache=False):
        ...
        try:
            params = config(section='corporations_canada')
            ...
            self.conn = None
            self.cache = None
            log_error("CorporationsCanada exception connecting to DB: " + str(error))
            raise
    ...
    def get_table_prefix(self, force_query_remote=False):
        if self.use_local_cache() and (not force_query_remote):
            ...
            return CORPORATION_CANADA_TABLE_PREFIX
    ...
    # return the table structure of a corpscanada table
    def get_corpscanada_table_struct(self, table, where=""):
    ...
    def cache_corpscanada_data(self, table, desc, rows, generate_individual_sql=False):
    ...
    # load all bc registries data for the specified corps into our in-mem cache
    def cache_corpscanada_corps(self, specific_corps, generate_individual_sql=False):
        if self.use_local_cache():
            self.cache_corpscanada_corp_tables(specific_corps, generate_individual_sql)
            self.cache_corpscanada_code_tables(generate_individual_sql)

    # load all bc registries data for the specified corps into our in-mem cache
    def cache_corpscanada_corp_tables(self, specific_corps, generate_individual_sql=False):
        if self.use_local_cache():
            ...
            for corp_nums_list in specific_corps_lists:
                ...
                party_rows = self.get_corpscanada_table(self.other_tables[0], corp_party_where, '', True, generate_individual_sql)
            ...
            for corp_nums_list in specific_corps_lists:
                ...
                event_rows = self.get_corpscanada_table(self.other_tables[1], event_where, '', True, generate_individual_sql)
            ...
            for ids_list in event_ids_lists:
                ...
                _rows = self.get_corpscanada_table(self.other_tables[2], filing_where, '', True, generate_individual_sql)
                _rows = self.get_corpscanada_table(self.other_tables[3], filing_where, '', True, generate_individual_sql)
            ...
            for corp_nums_list in specific_corps_lists:
                ...
                for corp_table in self.corp_tables:
                    _rows = self.get_corpscanada_table(corp_table, corp_num_where, '', True, generate_individual_sql)

                office_where = 'corp_num in (' + corp_nums_list + ')'
                office_rows = self.get_corpscanada_table(self.other_tables[4], office_where, '', True, generate_individual_sql)
    ...
    def cache_corpscanada_code_tables(self, generate_individual_sql=False):
        if self.use_local_cache():
            ...
            for code_table in self.code_tables:
                _rows = self.get_corpscanada_table(code_table, '', '', True, generate_individual_sql)
    ...
    def get_corpscanada_sql(self, table, sql, cache=False, generate_individual_sql=False):
        cursor = None
        try:
            cursor = self.conn.cursor()
            cursor.execute(sql)
            ...
            if self.use_local_cache() and cache:
                self.cache_corpscanada_data(table, desc, rows, generate_individual_sql)
            return rows
        except (Exception, psycopg2.DatabaseError) as error:
            print(error)
            print(traceback.print_exc())
    ...
    def get_corpscanada_table(self, table, where="", orderby="", cache=False, generate_individual_sql=False):
        ...
        return self.get_corpscanada_sql(table, sql, cache, generate_individual_sql)

    # get all records and return in an array of dicts
    # returns a zero-length array if none found
    # optionally takes a WHERE clause and ORDER BY clause (must be valid SQL)
    def get_corpscanada_corp_table(self, table, corp_num, where="", orderby="", cache=False, generate_individual_sql=False):
        subwhere = "corp_num = '" + corp_num + "'"
        if 0 < len(where):
            subwhere = subwhere + " AND " + where
        return self.get_corpscanada_table(table, subwhere, orderby, cache, generate_individual_sql)
