...
try:
    ...
    (initial_load_pipeline, success) = data_integration.pipelines.find_node(['initialization_and_load_tasks','corpscanada_corp_loader'])
    if success:
        corps_ct = 1
        prev_corps_ct = 0
        while 0 < corps_ct and corps_ct != prev_corps_ct:
            ...
            log_info("Ran corpscanada_corp_loader for " + str(corps_ct) + " corps")
    else:
        print("Pipeline not found")
        log_error("Pipeline not found for:" + "corpscanada_corp_loader")
except Exception as e:
    print("Exception", e)
    log_error("corpscanada_corp_loader processing exception: " + str(e))
    raise
