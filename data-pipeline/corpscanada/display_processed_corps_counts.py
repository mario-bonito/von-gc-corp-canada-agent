...
from corpscanada.corporationscanada import CorporationsCanada
...
def add_stats_to_dict(key, type):
    if not key in stats_dict:
        stats_dict[key] = {'corpscanada': 0, 'event_proc_inbound': 0, 'event_proc_outbound': 0, 'orgbook': 0}
...
with CorporationsCanada() as corporationscanada:
    # run this query against Corporations Canada database:
    sql1 = """
    select corp.corp_num,
           corp.corp_typ_cd,
           op_state.op_state_typ_cd
    from corporationscanada.corporation corp,
          corporationscanada.corp_state state,
       corporationscanada.corp_op_state op_state
    ...
    print("Get corp stats from Corporations Canada DB", datetime.datetime.now())
    ...
    for corpscanada_rec in corpscanada_recs:
        if corpscanada_rec['corp_typ_cd'] in CORP_TYPES_IN_SCOPE:
            key = corpscanada_rec['corp_typ_cd'] + ',' + corpscanada_rec['op_state_typ_cd']
            add_stats_to_dict(key, 'corpscanada')
            add_corp_to_dict(corpscanada_rec['corp_num'], key, 'corpscanada')
    #print(corpscanada_stats)
...
print("===========================")
print("Company Type,Company Status,corpscanada,event_proc_inbound,event_proc_outbound,orgbook")
for key, value in stats_dict.items():
    print(key + ',' + str(value['corpscanada']) + ',' + str(value['event_proc_inbound']) + ',' + str(value['event_proc_outbound']) + ',' + str(value['orgbook']))
print("===========================")
print("Incomplete data in OrgBook:")
print("===========================")
print("Company Type,Company Status,corpscanada,orgbook,orgbook_diff")
for key, value in stats_dict.items():
    if 0 != (value['corpscanada'] - value['orgbook']):
        print(key + ',' + str(value['corpscanada']) + ',' + str(value['orgbook']) + ',' + str(value['corpscanada'] - value['orgbook']))
print("===========================")
...
def add_missing_corps_to_queue(specific_corps):
    ...
    with BCRegistries() as corporationscanada:
        with EventProcessor() as event_processor:
            ...
            corps = corporationscanada.get_specific_corps(specific_corps)
            ...
            corps = corporationscanada.get_unprocessed_corp_events(prev_event_id, last_event_dt, max_event_id, max_event_dt, corps)
...
specific_corps_1 = []
missing_corps = {'corpscanada': 0, 'event_proc_inbound': 0, 'event_proc_outbound': 0, 'orgbook': 0}
specific_corps_2 = []
specific_corps_3 = []
incorrect_corps = {'corpscanada': 0, 'event_proc_inbound': 0, 'event_proc_outbound': 0, 'orgbook': 0}
for corp_num, corp_set in corps_dict.items():
    if corp_num in un_processed_outbound_corps:
        ...
        # skip records that are still outstanding
        pass
    elif 'corpscanada' in corp_set:
        if corp_set['orgbook'] != corp_set['corpscanada']:
        ...
        print(corp_num, 'in orgbook but not corpscanada, weird ...')
