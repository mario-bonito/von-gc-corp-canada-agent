...
posted to OrgBook (CREDENTIAL_LOG table).
    ...
    ./run-step.sh corpscanada/populate_audit_table.py
...
re-query the BC Reg database):
    ...
    ./run-step.sh corpscanada/generate-creds.py
...
if evp_corp_history_count != corpscanada_count:
    print("Error missing corps in Event Processor", corpscanada_count, evp_corp_history_count)
    if ERROR_THRESHOLD_COUNT < abs(evp_corp_history_count - corpscanada_count):
        log_error("Error missing corps in Event Processor: reg={} evp={}".format(corpscanada_count, evp_corp_history_count))
    else:
        log_warning("Warning missing corps in Event Processor: reg={} evp={}".format(corpscanada_count, evp_corp_history_count))
