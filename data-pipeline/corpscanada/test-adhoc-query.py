...
with CorporationsCanada(True) as corporationscanada:
    sql1 = """
            select corp_num, party_typ_cd, count(*) party_count
            from corporationscanada.corp_party
    ...
    sql2 = """
            ...
            from corporationscanada.corporation corp,
                 corporationscanada.corp_type typ,
    ...
    parties = corporationscanada.get_corpscanada_sql("parties", sql1, cache=True)

    print("Get active BC corporations", datetime.datetime.now())
    corps = corporationscanada.get_corpscanada_sql("corps", sql2, cache=True)

    print("Do a query to verify local cached data", datetime.datetime.now())
    p_count = corporationscanada.get_adhoc_query("select count(*) count from parties")
    print("Party count =", p_count, datetime.datetime.now())
    c_count = corporationscanada.get_adhoc_query("select count(*) count from corps")
    ...
    sql3 = """
           ...
           """
    corps = corporationscanada.get_adhoc_query(sql3)
    print_header = False
    histogram = {}
    for corp in corps:
        if not print_header:
